...
########################################################################
#                                                                       
# LIB_download_images.php     POP3 Mail Routines    图片下载
#                                                                       
...
# FUNCTIONS                                                             
#                                                                       
#   download_binary_file()    下载安全二进制图片
#                     A binary-safe download routine for images
#                                                                       
#   mkpath($path)       给下载的图片创建一个存放的路径地址
#                     Createa a directory path for downloaded images
#                                                                       
#   download_images_for_page($target)     从给出的网页中下载所有图片
#                     Downloads all images for a given target webpage
#                                                                       
#-----------------------------------------------------------------------

include_once("LIB_parse.class.php");               # include parse library
include_once("LIB_http.class.php");                # include curl library
include_once("LIB_resolve_addresses.class.php");   # include address resolution分解 library
...
***********************************************************************/
function download_binary_file($file, $ref,$cookie_file){
    # Open a PHP/CURL session
    $s = curl_init();
    
    # Configure the CURL command
    curl_setopt($s, CURLOPT_URL, $file);               // Define target site
    curl_setopt($s, CURLOPT_RETURNTRANSFER, TRUE);     // Return in string   将curl_exec()获取的信息以文件流的形式返回，而不是直接输出。
    curl_setopt($s, CURLOPT_BINARYTRANSFER, 1);        // Indicate binary transfer   在启用CURLOPT_RETURNTRANSFER的时候，返回原生的（Raw）输出
	curl_setopt($s, CURLOPT_REFERER, $ref);            // Referer value     在HTTP请求头中"Referer: "的内容。
	curl_setopt($s, CURLOPT_SSL_VERIFYPEER, FALSE);    // No certificate    禁用后cURL将终止从服务端进行验证
	curl_setopt($s, CURLOPT_FOLLOWLOCATION, TRUE);     // Follow redirects  将服务器返回的"Location: "放在header中递归的返回给服务器
	curl_setopt($s, CURLOPT_MAXREDIRS, 4);             // Limit redirections重定向 to four
    curl_setopt($s,CURLOPT_COOKIEFILE, $cookie_file);//写入cookie
    curl_setopt($s,CURLOPT_COOKIEJAR, $cookie_file);//写入cookie
...
function download_images_for_page($target)
    ...
    function write_cookie_file($file,$ref,$cookie_file){
    # Open a PHP/CURL session
    $s = curl_init();
